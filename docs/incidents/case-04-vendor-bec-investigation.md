...
Using the project's risk scoring model
([`risk-scoring-model.md`](../detections/risk-scoring-model.md)):

| Signal | Points | Present in CASE-04? |
|---|---|---|
| Reply-To domain mismatch | +20 | Yes (Detection 1) |
| Urgency language present | +20 | Yes (Detection 2) |
| Suspicious/unverifiable URL | +15 | Yes, domain unresolvable, could not be confirmed safe or malicious |
| Financial/payment change request in body | +25 | Yes, requests a banking detail change |
| Two independent detections agreeing | +10 | Yes, Detection 1 and Detection 2 both fired |
| **Total** | **90** | **Critical** |

This matches the dataset's own pre-labeled `severity = Critical` and
`expected_verdict = BEC-style phishing / payment fraud lure`, and is the
highest score of all 4 cases in this dataset (see the model doc for the
full comparison).

## 5. Respond

All response actions below are **simulated**, appropriate for a synthetic
training lab and consistent with the project's safety requirements. No
real infrastructure, mailbox, or vendor was contacted.

- **Simulated quarantine**: the email would be pulled from the recipient's
  inbox and held for review, rather than left actionable.
- **Simulated URL block**: `northwind-payment-update.example.invalid`
  would be added to an outbound URL block list, preventing anyone from
  reaching it even if they clicked before quarantine.
- **Simulated sender/domain flag**: `northwind-supplies.example.invalid`
  and `northwind-payments.example.invalid` would be flagged for extra
  scrutiny on any future mail, since the mismatch pattern itself is the
  core indicator here, not just this one message.
- **Simulated finance verification**: because this case impersonates a
  vendor and requests a banking change, the correct real-world response
  is always out-of-band verification, calling the vendor using a phone
  number from prior legitimate records, not any contact info in the
  suspicious email itself, before any payment or banking detail is
  changed.
- **Simulated user notification**: the (hypothetical) recipient would be
  informed why the email was flagged, in plain terms, so they recognize
  the pattern if it recurs.

## 6. Close

**Disposition**: True Positive, confirmed BEC/payment fraud lure.

**Basis for closure**: two independent detections fired and agreed,
manual validation of the email body confirmed a genuine vendor-impersonation
and payment-redirection pattern, enrichment on both IOCs returned no data
(correctly interpreted as "unable to verify," not "cleared"), and the risk
score of 90 (Critical) reflects the combined signal.

**Follow-up items generated by this investigation**:
- Consider adding an authentication-failure signal to the risk scoring
  model (see `risk-scoring-model.md`, "What I'd Improve Next").
- No legitimate/clean sample exists yet in this dataset; false-positive
  behavior of both detections and this scoring model remains unverified
  against real, benign mail.
